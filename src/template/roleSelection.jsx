...
const RoleSelection = () => {
    ...
    useEffect(() => {
        const fetchVideos = async () => {
            if (!roleCode) {
                console.error('Role code is missing or invalid');
                return; // Exit if roleCode is not set
            }

            console.log('Fetching videos for role code:', roleCode);
            ...
            try {
                const response = await fetch(`${env.baseURL}/api/videos/role/${roleCode}`);
        ...
        // Make sure to call fetchVideos with a valid roleCode
        if (roleCode) {
            fetchVideos();
        } else {
            console.error('roleCode is invalid or missing');
        }



        fetchVideos(roleCode);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [roleCode]); // you don't need to add 'fetching' into dependencies
